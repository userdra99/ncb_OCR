#!/usr/bin/env python3
"""
Generate NCB API JSON files from extracted receipt data.

This script:
1. Finds all processed receipt jobs in Redis
2. Extracts the OCR data for each
3. Generates NCB API compliant JSON files
4. Uploads them to Google Shared Drive
"""
...
async def get_all_processed_jobs() -> List[Dict[str, Any]]:
    ...
    try:
        await queue_service.connect()  # Initialize Redis connection
    ...
    while True:
        ...
        for key in keys:
            # Keys are already strings if decode_responses=True
...
async def main():
    ...
    logger.info("="*70)
    logger.info("NCB API JSON Generation from Extracted Receipts")
    logger.info("="*70)
    ...
    logger.info("\n" + "="*70)
    logger.info(f"Uploading {len(jobs_to_upload)} files to Google Shared Drive...")
    logger.info("="*70)
    ...
    summary_path = Path("/app/data/ncb_api_json/upload_summary.json")
