#!/usr/bin/env python3
"""
Generate sample claim JSON files and upload to Google Drive using OAuth.
Uses user's Gmail OAuth token (which includes Drive scope).
"""
...
def get_drive_service():
    ...
    # Load OAuth token (same as Gmail)
...
async def upload_to_drive(filepath: Path, folder_id: str):
    """
    Upload JSON file to Google Drive using OAuth.
    ...
    try:
        ...
        # Upload using OAuth (user's storage quota)
...
async def main():
    """Main entry point."""
    print("=" * 70)
    print("📄 Claims JSON Generator - OAuth Drive Upload")
    print("=" * 70)
    print()
    ...
    for idx, claim in enumerate(claims, 1):
        try:
            ...
            # Upload to Drive via OAuth
            file_id = await upload_to_drive(filepath, drive_folder_id)
    ...
    print()
    print("✅ All claims uploaded to Google Drive: Claims Archive")
